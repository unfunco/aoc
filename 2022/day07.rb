...
# :nodoc:
class String
  # Returns a boolean indicative of whether the string is a valid integer.
...
File.read("#{__dir__}/input/day07").lines do |line|
  ...
  # lines we care about, ls commands on their own are not very useful.
  if %w[$ cd /] == [a, b, c]
    cwd = Pathname.new('/')
  elsif %w[$ cd] == [a, b]
    cwd += c
...
sizes = lambda do |dir|
  files.select do |f|
    f.to_s.start_with?(dir.to_s)
  end.values.sum
end

puts fs.keys.map { |dir| sizes[dir] }.filter { |size| size <= 100_000 }.sum
...
# total amount of usexd space) is 48381165; this means that the size of the
...
# enough space. However, directories d and / are both big enough!
...
used = files.values.sum
puts fs.keys.map { |dir| sizes[dir] }.filter { |size|
  used - size <= 40_000_000
}.min
